...
workflow FASTQstats {
    ...
    if (defined(exclude_len_threshold)) {
        call FQU.FilterByLen { input: fastq = fastq, exclude_len_threshold = select_first([exclude_len_threshold])}
    }

    call FQU.Stats { input: fastq = select_first([FilterByLen.res, fastq]), seq_type = seq_type }
